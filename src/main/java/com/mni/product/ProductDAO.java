...
public class ProductDAO {
   ...
   /** 카테고리별 상품 내역 출력 메서드 */
      ...
      /** 메뉴 신상품 보기 */
       public ArrayList<ProductDTO> newProduct(String pet_prod, int cp, int pageCnt) {
        try {
           conn = com.mni.db.MniDB.getConn();
           int start = (cp-1)*pageCnt+1;
                          int end = cp*pageCnt;
           String sql = "select * from"
                       + "(select rownum as rnum, a.* from"
                       + "(select * from product where prod_pet = ? and "
             + "prod_date between sysdate-14 and sysdate order by prod_date desc) a) "
                       + "where rnum>=? and rnum<=?";
      ...
      /**베스트 관련 메서드*/
      public ArrayList<ProductDTO> indexBest(String pet_prod){
        try {
           conn=com.mni.db.MniDB.getConn();
           String sql="select * from "
                 + "(select rownum as rnum,a.* from "
                 + "(select * from product where prod_pet = ? order by prod_sale desc) a) b "
                 + "where rnum>=(1-1)*8+1 and rnum<=1*8";
           ps=conn.prepareStatement(sql);
       ps.setString(1, pet_prod);
           rs=ps.executeQuery();
