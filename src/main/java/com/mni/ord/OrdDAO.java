...
public class OrdDAO {
    ...
    public ArrayList<OrdDTO> UserOrderSelect(int idx){
          try {
             conn = com.mni.db.MniDB.getConn();
             String sql = "select order_date, prod_title_img, prod_name,o.order_idx,order_type,order_price "
             ...
             while(rs.next()) {
                ...
                String order_type = rs.getString("order_type");
                int order_price = rs.getInt("order_price");
                OrdDTO dto = new OrdDTO(order_idx, order_type, order_date, order_price, prod_title_img, prod_name);
                arr.add(dto);
             }
             return arr;
          }catch(Exception e) {
	...
	public int adminSales(String startDate,String endDate) {
		try {
			conn=com.mni.db.MniDB.getConn();
			String sql="select sum(order_price) from ord where to_char(order_date,'YYYYMMDD')>=? and to_char(order_date,'YYYYMMDD')<=? and order_state = '구매확정'";
			ps=conn.prepareStatement(sql);
