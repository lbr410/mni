...
public class ReviewDAO {
   ...
   public ArrayList<ReviewDTO> reviewSearch(String prod_name,int cp,int pageCnt){
	   try {
		   ...
		   String sql = "select * from "
		   		...
		   		+ "(select u.user_id,r.review_idx,p.prod_idx,p.prod_name,r.review_content,r.review_date,r.review_img from review r,product p,userinfo u where r.prod_idx = p.prod_idx and u.user_idx = r.user_idx and p.prod_name = ? order by review_idx desc) a) b "
		   		+ "where rnum>=? and rnum<=?";
		   ps=conn.prepareStatement(sql);
   ...
   public ArrayList<ReviewDTO> getReview(int cp,int pageCnt){
	   try {
		   ...
		   String sql = "select * from "
		   		+ "(select rownum as rnum,a.* from "
		   		+ "(select u.user_id,r.review_idx,p.prod_idx,p.prod_name,r.review_content,r.review_date,r.review_img from review r,product p,userinfo u where r.prod_idx = p.prod_idx and u.user_idx = r.user_idx order by review_idx desc) a) b "
		   		+ "where rnum>=? and rnum<=?";
		   ps=conn.prepareStatement(sql);
		   ps.setInt(1, start);
			ps.setInt(2, end);
